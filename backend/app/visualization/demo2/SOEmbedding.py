            ...
            for k in range(j + 1, n):
                if labels[A_order[i, 0]] == labels[A_order[i, j]] and labels[A_order[i, 0]] == labels[A_order[i, k]]:
                    if disgraph[A_order[i, 0], A_order[i, j]] + Del - disgraph[A_order[i, 0], A_order[i, k]] > 0:
                        ...
                        x_i = (
                            RS(theta_li, scal_li) @ delx[A_order[i, 0], :].transpose()).transpose() + C[li, :]
                        x_j = (
                            RS(theta_lj, scal_lj) @ delx[A_order[i, j], :].transpose()).transpose() + C[lj, :]
                        x_k = (
                            RS(theta_lk, scal_lk) @ delx[A_order[i, k], :].transpose()).transpose() + C[lk, :]
